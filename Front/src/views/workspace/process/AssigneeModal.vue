<template>
  <el-dialog title="转办" :visible.sync="show" width="500px" v-bind="$attrs" v-on="$listeners" @close="handleCancel">
    <el-form v-loading="loading" ref="formRef" :model="formValue" :rules="rules">
      <el-form-item label="转办人" prop="transferUserInfo" required>
        <el-button size="mini" @click="handleShowOrgPicker"><i class="el-icon-user"></i> 选择人员</el-button>
        <el-tag v-if="transferUserInfoUserName" :style="{marginLeft: '10px'}" size="small">{{ transferUserInfoUserName  }}</el-tag>
      </el-form-item>
      <el-form-item prop="comments">
        <el-input type="textarea" v-model="formValue.comments" placeholder="转办意见" maxlength="255" rows="4" show-word-limit />
      </el-form-item>
    </el-form>
    <template #footer>
