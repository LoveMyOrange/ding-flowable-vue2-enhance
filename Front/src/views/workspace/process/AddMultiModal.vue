<template>
  <el-dialog title="加签" :visible.sync="show" width="500px" v-bind="$attrs" v-on="$listeners" @close="handleCancel">
    <el-form v-loading="loading" ref="formRef" :model="formValue" :rules="rules">
      <el-form-item label="给谁加签" prop="multiAddUserInfo" required>
        ...
        <el-tag v-if="multiAddUserInfoUserName" :style="{marginLeft: '10px'}" size="small">{{ multiAddUserInfoUserName  }}</el-tag>
      </el-form-item>
      <el-form-item prop="comments">
        <el-input type="textarea" v-model="formValue.comments" placeholder="加签意见" maxlength="255" rows="4" show-word-limit />
      </el-form-item>
    </el-form>
    <template #footer>
